...
exports.escapeNameString = escapeNameString;
...
exports.shouldEncodedOrEscapeAddresses = shouldEncodedOrEscapeAddresses;
...
function getAddressesHeader(addrArray) {
  if (shouldEncodedOrEscapeAddresses(addrArray)) {
    return utf8encode(
      addrArray.map(({ name, address }) => `${name} <${address}>`).join(', '),
    );
  }
  return addrArray
    .map(({ name, address }) => `${escapeNameString(name)} <${address}>`)
    .join(', ');
}
exports.getAddressesHeader = getAddressesHeader;
